...
from tools.renderer_synthetic_demo import letterbox_bgr_to_rgb, letterbox_rgb_to_rgb
...
class RendererInputTests(unittest.TestCase):
    ...
    def test_rgb_letterbox_keeps_channel_order_and_reuses_canvas(self) -> None:
        source_rgb = np.zeros((2, 4, 3), dtype=np.uint8)
        source_rgb[:, :] = [30, 20, 10]
        destination_rgb = np.full((4, 4, 3), 255, dtype=np.uint8)
        identity = id(destination_rgb)

        placement = letterbox_rgb_to_rgb(source_rgb, destination_rgb, cv2)

        self.assertEqual(placement, (0, 1, 4, 2))
        self.assertEqual(id(destination_rgb), identity)
        np.testing.assert_array_equal(destination_rgb[1:3], np.broadcast_to([30, 20, 10], (2, 4, 3)))

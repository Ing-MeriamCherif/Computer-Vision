...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `main.py` | A | Shared | Top-level entry point; launches the P123 Material 3 UI | User / CLI | YES | NO | NO | KEEP (authoritative entry point) |
...
| `geometry/camera_worker.py` | B | Shared | `CameraCaptureWorker`, `LatestFrameSlot` (capacity=1, monotonic timestamps, dropped frame counting) | `p123_live_runtime.py`, `physical_camera_gate.py` | YES | NO | NO | KEEP |
| `geometry/depth_provider.py` | A | P2 | `DepthAnythingProvider` (Depth Anything V2 Small local inference, FP32/FP16 CUDA) | `p123_live_runtime.py`, `physical_camera_gate.py` | YES | NO | NO | KEEP (normalize interface: `backend_name`, `load`, `compute`, `last_diagnostics`) |
| `geometry/colleague_depth.py` | A | P2 | `ColleagueDepthProvider` (adapter for Meriam's `feature/depth` model under `integrations/`) | `p123_live_runtime.py`, `physical_camera_gate.py` | YES (when selected) | NO | NO | KEEP (preserves colleague provenance) |
...
| `geometry/normals.py` | A | P3 | `estimate_normals`, `normals_to_rgb`, `geometry_from_depth_state` (discontinuity-aware normals) | `cuda_backend.py`, `p123_live_runtime.py`, `physical_camera_gate.py` | YES | NO | NO | KEEP |
| `geometry/cuda_backend.py` | A | P3 | `TorchGeometryBackend` (CUDA tensor backprojection & normal estimation) | `p123_live_runtime.py`, `physical_camera_gate.py`, tests | YES | NO | NO | KEEP |
| `geometry/motion.py` | A | P3 | `MotionState`, `OpticalFlowProvider`, `OpenCVFlowProvider` (DIS/Farneback forward/backward flow) | `temporal.py`, `p123_live_runtime.py` | YES | NO | NO | KEEP |
...
| `geometry/hand_control.py` | A | P1 | `HandControlEngine`, `TrackedHand`, `GestureState` (MediaPipe 21-landmark tracking, One-Euro filter, 2-hand support) | `p123_live_runtime.py`, `physical_camera_gate.py` | YES | NO | NO | KEEP |
| `geometry/async_pipeline.py` | B | Shared | `DepthWorker`, `LatestFrameBuffer`, `LatestDepthBuffer` (decoupled inference thread) | `p123_live_runtime.py` | YES | NO | NO | KEEP |
...
| `geometry/lighting.py` | A | P4 | `LightState`, `shade_geometry`, `render_volumetric_scattering`, `light_from_palm` | `p123/views/relight/` | YES | NO | NO | **KEEP UNTOUCHED** (Absolute P4 rule: black box) |
| `geometry/persistent.py` | D | P3 / Shared | `SurfelMap`, `PersistentGeometryMapper`, `AdvancedGeometryEngine` (Phase 5 persistent mapping) | `persistent_worker.py` | NO (experimental) | NO | NO | KEEP (isolate from default P123 path) |
| `geometry/persistent_worker.py` | D | Shared | `PersistentMapWorker` (async sidecar thread for persistent mapping) | `p123_live_runtime.py` | NO (experimental) | NO | NO | KEEP (isolate from default P123 path) |
| `geometry/pose.py` | D | P3 | `PoseEstimator`, `CameraPoseState`, `PoseEstimateResult` (PnP-RANSAC relative camera pose) | `persistent.py`, `p123_live_runtime.py` | NO (experimental) | NO | NO | KEEP (isolate from default P123 path) |
